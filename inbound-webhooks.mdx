...
You can also attach action buttons to the resulting notification or activity that open the source service (the GitHub run, the Sentry issue, the Vercel deployment, and so on) or trigger a backend webhook. See the [action buttons section](/#action-buttons) on the quickstart for the field reference.
