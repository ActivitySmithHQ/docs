...
- `redirection` opens an HTTPS URL or runs an Apple Shortcut with a `shortcuts://` URL on normal tap.
- `actions` (up to 4) appear when users expand the notification. `open_url` actions support HTTPS and `shortcuts://` URLs; `webhook` actions call your backend.
...
- `open_url`: opens an HTTPS URL or runs an Apple Shortcut with a `shortcuts://` URL.
...
| `url`    | yes          | For `open_url`, use an HTTPS or `shortcuts://` URL. For `webhook`, use an HTTPS endpoint. |
...
tied to the work. You can also use a `shortcuts://` URL to run an Apple
Shortcut, for example to open an app on your iPhone.
